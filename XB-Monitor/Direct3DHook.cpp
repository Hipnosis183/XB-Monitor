...
LPDirect3DCreate9 HDirect3DCreate9 = NULL;

static LPDIRECT3D9 pD3D;
static LPDIRECT3DDEVICE9 pD3Dev;

struct FAR HookIDirect3DDevice9;
HookIDirect3DDevice9 D3DevWrapper, * pD3DevWrapper = &D3DevWrapper;
...
{
	pD3D = HDirect3DCreate9(SDKVersion);
...
HRESULT HookIDirect3DDevice9::CreateAdditionalSwapChain(LPVOID _this, D3DPRESENT_PARAMETERS* pPresentationParameters, IDirect3DSwapChain9** pSwapChain)
{
...
HRESULT HookIDirect3DDevice9::Present(LPVOID _this, CONST RECT* pSourceRect, CONST RECT* pDestRect, HWND hDestWindowOverride, CONST RGNDATA* pDirtyRegion)
{
	return pD3Dev->Present(pSourceRect, pDestRect, hDestWindowOverride, pDirtyRegion);
}

HRESULT HookIDirect3DDevice9::GetBackBuffer(LPVOID _this, UINT iSwapChain, UINT iBackBuffer, D3DBACKBUFFER_TYPE Type, IDirect3DSurface9** ppBackBuffer)
{
...
HRESULT HookIDirect3DDevice9::CreateVolumeTexture(LPVOID _this, UINT Width, UINT Height, UINT Depth, UINT Levels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, IDirect3DVolumeTexture9** ppVolumeTexture, HANDLE* pSharedHandle)
{
	return pD3Dev->CreateVolumeTexture(Width, Height, Depth, Levels, Usage, Format, Pool, ppVolumeTexture, pSharedHandle);
}

HRESULT HookIDirect3DDevice9::CreateCubeTexture(LPVOID _this, UINT EdgeLength, UINT Levels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, IDirect3DCubeTexture9** ppCubeTexture, HANDLE* pSharedHandle)
{
	return pD3Dev->CreateCubeTexture(EdgeLength, Levels, Usage, Format, Pool, ppCubeTexture, pSharedHandle);
}

HRESULT HookIDirect3DDevice9::CreateVertexBuffer(LPVOID _this, UINT Length, DWORD Usage, DWORD FVF, D3DPOOL Pool, IDirect3DVertexBuffer9** ppVertexBuffer, HANDLE* pSharedHandle)
{
	return pD3Dev->CreateVertexBuffer(Length, Usage, FVF, Pool, ppVertexBuffer, pSharedHandle);
}

HRESULT HookIDirect3DDevice9::CreateIndexBuffer(LPVOID _this, UINT Length, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, IDirect3DIndexBuffer9** ppIndexBuffer, HANDLE* pSharedHandle)
{
	return pD3Dev->CreateIndexBuffer(Length, Usage, Format, Pool, ppIndexBuffer, pSharedHandle);
}

HRESULT HookIDirect3DDevice9::CreateRenderTarget(LPVOID _this, UINT Width, UINT Height, D3DFORMAT Format, D3DMULTISAMPLE_TYPE MultiSample, DWORD MultisampleQuality, BOOL Lockable, IDirect3DSurface9** ppSurface, HANDLE* pSharedHandle)
{
	return pD3Dev->CreateRenderTarget(Width, Height, Format, MultiSample, MultisampleQuality, Lockable, ppSurface, pSharedHandle);
}

HRESULT HookIDirect3DDevice9::CreateDepthStencilSurface(LPVOID _this, UINT Width, UINT Height, D3DFORMAT Format, D3DMULTISAMPLE_TYPE MultiSample, DWORD MultisampleQuality, BOOL Discard, IDirect3DSurface9** ppSurface, HANDLE* pSharedHandle)
{
...
HRESULT HookIDirect3DDevice9::UpdateSurface(LPVOID _this, IDirect3DSurface9* pSourceSurface, CONST RECT* pSourceRect, IDirect3DSurface9* pDestinationSurface, CONST POINT* pDestPoint)
{
	return pD3Dev->UpdateSurface(pSourceSurface, pSourceRect, pDestinationSurface, pDestPoint);
}

HRESULT HookIDirect3DDevice9::UpdateTexture(LPVOID _this, IDirect3DBaseTexture9* pSourceTexture, IDirect3DBaseTexture9* pDestinationTexture)
{
	return pD3Dev->UpdateTexture(pSourceTexture, pDestinationTexture);
}

HRESULT HookIDirect3DDevice9::GetRenderTargetData(LPVOID _this, IDirect3DSurface9* pRenderTarget, IDirect3DSurface9* pDestSurface)
{
	return pD3Dev->GetRenderTargetData(pRenderTarget, pDestSurface);
}

HRESULT HookIDirect3DDevice9::GetFrontBufferData(LPVOID _this, UINT iSwapChain, IDirect3DSurface9* pDestSurface)
{
	return pD3Dev->GetFrontBufferData(iSwapChain, pDestSurface);
}

HRESULT HookIDirect3DDevice9::StretchRect(LPVOID _this, IDirect3DSurface9* pSourceSurface, CONST RECT* pSourceRect, IDirect3DSurface9* pDestSurface, CONST RECT* pDestRect, D3DTEXTUREFILTERTYPE Filter)
{
	return pD3Dev->StretchRect(pSourceSurface, pSourceRect, pDestSurface, pDestRect, Filter);
}

HRESULT HookIDirect3DDevice9::ColorFill(LPVOID _this, IDirect3DSurface9* pSurface, CONST RECT* pRect, D3DCOLOR color)
{
	return pD3Dev->ColorFill(pSurface, pRect, color);
}

HRESULT HookIDirect3DDevice9::CreateOffscreenPlainSurface(LPVOID _this, UINT Width, UINT Height, D3DFORMAT Format, D3DPOOL Pool, IDirect3DSurface9** ppSurface, HANDLE* pSharedHandle)
{
	return pD3Dev->CreateOffscreenPlainSurface(Width, Height, Format, Pool, ppSurface, pSharedHandle);
}

HRESULT HookIDirect3DDevice9::SetRenderTarget(LPVOID _this, DWORD RenderTargetIndex, IDirect3DSurface9* pRenderTarget)
{
	return pD3Dev->SetRenderTarget(RenderTargetIndex, pRenderTarget);
}

HRESULT HookIDirect3DDevice9::GetRenderTarget(LPVOID _this, DWORD RenderTargetIndex, IDirect3DSurface9** ppRenderTarget)
{
	return pD3Dev->GetRenderTarget(RenderTargetIndex, ppRenderTarget);
}

HRESULT HookIDirect3DDevice9::SetDepthStencilSurface(LPVOID _this, IDirect3DSurface9* pNewZStencil)
{
	return pD3Dev->SetDepthStencilSurface(pNewZStencil);
}

HRESULT HookIDirect3DDevice9::GetDepthStencilSurface(LPVOID _this, IDirect3DSurface9** ppZStencilSurface)
{
...
HRESULT HookIDirect3DDevice9::Clear(LPVOID _this, DWORD Count, CONST D3DRECT* pRects, DWORD Flags, D3DCOLOR Color, FLOAT Z, DWORD Stencil)
{
	return pD3Dev->Clear(Count, pRects, Flags, Color, Z, Stencil);
}

HRESULT HookIDirect3DDevice9::SetTransform(LPVOID _this, D3DTRANSFORMSTATETYPE State, CONST D3DMATRIX* pMatrix)
{
	return pD3Dev->SetTransform(State, pMatrix);
}

HRESULT HookIDirect3DDevice9::GetTransform(LPVOID _this, D3DTRANSFORMSTATETYPE State, D3DMATRIX* pMatrix)
{
	return pD3Dev->GetTransform(State, pMatrix);
}

HRESULT HookIDirect3DDevice9::MultiplyTransform(LPVOID _this, D3DTRANSFORMSTATETYPE Type, CONST D3DMATRIX* pMatrix)
{
	return pD3Dev->MultiplyTransform(Type, pMatrix);
}

HRESULT HookIDirect3DDevice9::SetViewport(LPVOID _this, CONST D3DVIEWPORT9* pViewport)
{
	return pD3Dev->SetViewport(pViewport);
}

HRESULT HookIDirect3DDevice9::GetViewport(LPVOID _this, D3DVIEWPORT9* pViewport)
{
	return pD3Dev->GetViewport(pViewport);
}

HRESULT HookIDirect3DDevice9::SetMaterial(LPVOID _this, CONST D3DMATERIAL9* pMaterial)
{
	return pD3Dev->SetMaterial(pMaterial);
}

HRESULT HookIDirect3DDevice9::GetMaterial(LPVOID _this, D3DMATERIAL9* pMaterial)
{
	return pD3Dev->GetMaterial(pMaterial);
}

HRESULT HookIDirect3DDevice9::SetLight(LPVOID _this, DWORD Index, CONST D3DLIGHT9* pLight)
{
	return pD3Dev->SetLight(Index, pLight);
}

HRESULT HookIDirect3DDevice9::GetLight(LPVOID _this, DWORD Index, D3DLIGHT9* pLight)
{
	return pD3Dev->GetLight(Index, pLight);
}

HRESULT HookIDirect3DDevice9::LightEnable(LPVOID _this, DWORD Index, BOOL Enable)
{
...
HRESULT HookIDirect3DDevice9::CreateStateBlock(LPVOID _this, D3DSTATEBLOCKTYPE Type, IDirect3DStateBlock9** ppSB)
{
...
HRESULT HookIDirect3DDevice9::GetTextureStageState(LPVOID _this, DWORD Stage, D3DTEXTURESTAGESTATETYPE Type, DWORD* pValue)
{
...
HRESULT HookIDirect3DDevice9::DrawPrimitive(LPVOID _this, D3DPRIMITIVETYPE PrimitiveType, UINT StartVertex, UINT PrimitiveCount)
{
	return pD3Dev->DrawPrimitive(PrimitiveType, StartVertex, PrimitiveCount);
}

HRESULT HookIDirect3DDevice9::DrawIndexedPrimitive(LPVOID _this, D3DPRIMITIVETYPE PrimitiveType, INT BaseVertexIndex, UINT MinVertexIndex, UINT NumVertices, UINT startIndex, UINT primCount)
{
	return pD3Dev->DrawIndexedPrimitive(PrimitiveType, BaseVertexIndex, MinVertexIndex, NumVertices, startIndex, primCount);
}

HRESULT HookIDirect3DDevice9::DrawPrimitiveUP(LPVOID _this, D3DPRIMITIVETYPE PrimitiveType, UINT PrimitiveCount, CONST VOID* pVertexStreamZeroData, UINT VertexStreamZeroStride)
{
	return pD3Dev->DrawPrimitiveUP(PrimitiveType, PrimitiveCount, pVertexStreamZeroData, VertexStreamZeroStride);
}

HRESULT HookIDirect3DDevice9::DrawIndexedPrimitiveUP(LPVOID _this, D3DPRIMITIVETYPE PrimitiveType, UINT MinVertexIndex, UINT NumVertices, UINT PrimitiveCount, CONST VOID* pIndexData, D3DFORMAT IndexDataFormat, CONST VOID* pVertexStreamZeroData, UINT VertexStreamZeroStride)
{
...
HRESULT HookIDirect3DDevice9::ProcessVertices(LPVOID _this, UINT SrcStartIndex, UINT DestIndex, UINT VertexCount, IDirect3DVertexBuffer9* pDestBuffer, IDirect3DVertexDeclaration9* pVertexDecl, DWORD Flags)
{
	return pD3Dev->ProcessVertices(SrcStartIndex, DestIndex, VertexCount, pDestBuffer, pVertexDecl, Flags);
}

HRESULT HookIDirect3DDevice9::CreateVertexDeclaration(LPVOID _this, CONST D3DVERTEXELEMENT9* pVertexElements, IDirect3DVertexDeclaration9** ppDecl)
{
...
HRESULT HookIDirect3DDevice9::CreateVertexShader(LPVOID _this, CONST DWORD* pFunction, IDirect3DVertexShader9** ppShader)
{
...
HRESULT HookIDirect3DDevice9::SetVertexShaderConstantF(LPVOID _this, UINT StartRegister, CONST FLOAT* pConstantData, UINT Vector4fCount)
{
	return pD3Dev->SetVertexShaderConstantF(StartRegister, pConstantData, Vector4fCount);
}

HRESULT HookIDirect3DDevice9::GetVertexShaderConstantF(LPVOID _this, UINT StartRegister, FLOAT* pConstantData, UINT Vector4fCount)
{
...
HRESULT HookIDirect3DDevice9::GetStreamSource(LPVOID _this, UINT StreamNumber, IDirect3DVertexBuffer9** ppStreamData, UINT* pOffsetInBytes, UINT* pStride)
{
...
HRESULT HookIDirect3DDevice9::CreatePixelShader(LPVOID _this, CONST DWORD* pFunction, IDirect3DPixelShader9** ppShader)
{
...
HRESULT HookIDirect3DDevice9::SetPixelShaderConstantF(LPVOID _this, UINT StartRegister, CONST FLOAT* pConstantData, UINT Vector4fCount)
{
	return pD3Dev->SetPixelShaderConstantF(StartRegister, pConstantData, Vector4fCount);
}

HRESULT HookIDirect3DDevice9::GetPixelShaderConstantF(LPVOID _this, UINT StartRegister, FLOAT* pConstantData, UINT Vector4fCount)
{
	return pD3Dev->GetPixelShaderConstantF(StartRegister, pConstantData, Vector4fCount);
}

HRESULT HookIDirect3DDevice9::SetPixelShaderConstantI(LPVOID _this, UINT StartRegister, CONST INT* pConstantData, UINT Vector4iCount)
{
	return pD3Dev->SetPixelShaderConstantI(StartRegister, pConstantData, Vector4iCount);
}

HRESULT HookIDirect3DDevice9::GetPixelShaderConstantI(LPVOID _this, UINT StartRegister, INT* pConstantData, UINT Vector4iCount)
{
...
HRESULT HookIDirect3DDevice9::GetPixelShaderConstantB(LPVOID _this, UINT StartRegister, BOOL* pConstantData, UINT BoolCount)
{
	return pD3Dev->GetPixelShaderConstantB(StartRegister, pConstantData, BoolCount);
}

HRESULT HookIDirect3DDevice9::DrawRectPatch(LPVOID _this, UINT Handle, CONST FLOAT* pNumSegs, CONST D3DRECTPATCH_INFO* pRectPatchInfo)
{
	return pD3Dev->DrawRectPatch(Handle, pNumSegs, pRectPatchInfo);
}

HRESULT HookIDirect3DDevice9::DrawTriPatch(LPVOID _this, UINT Handle, CONST FLOAT* pNumSegs, CONST D3DTRIPATCH_INFO* pTriPatchInfo)
{
